...
import time
import threading
import csp_python.libcsp_py3 as csp
from typing import Any, Callable


def printer(node: str, color: str) -> Callable:
    def f(inp: str) -> None:
        print('{color}[{name}]: {inp}\033[0m'.format(color=color, name=node.upper(), inp=inp))
    ...
    while 1:
        ...
        while (packet := csp.read(conn, 50)) is not None:
            if csp.conn_dport(conn) == port:
                _print('Recieved on {port}: {data}'.format(port=port, data=csp.packet_get_data(packet).decode('utf-8')))
            else:
